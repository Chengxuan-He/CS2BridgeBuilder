...
in both objects. A tower width may not be compared with a road-surface width in one object and an
outer railing or visible-deck width in the other. Asymmetric roads must retain separate recorded left
and right boundaries where a single total width would hide the mismatch.
