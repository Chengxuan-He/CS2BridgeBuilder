...
internal static class BridgeBasePrice
{
    ...
    internal static bool TryCalculate(long prototypeBridge, long prototypeRoads,
        long upper, long lower, long elevatedFloor, out long offset, out long total)
    {
        offset = total = 0;
        if (prototypeBridge < 0 || prototypeRoads < 0 || upper < 0 || lower < 0 || elevatedFloor < 0) return false;
        try
        {
            offset = checked(prototypeBridge - 3L * prototypeRoads);
            total = Math.Max(elevatedFloor, checked(offset + 3L * checked(upper + lower)));
