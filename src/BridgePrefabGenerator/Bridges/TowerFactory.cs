...
internal sealed class TowerFactory
{
    ...
    {
        {
            ...
            foreach (var info in parts)
            {
                ...
                var widened = Widen(
                    (RenderPrefab)info.m_Mesh!, name, meshes.Count, partExtra);
    ...
    /// </summary>
    private RenderPrefab? Widen(RenderPrefab original, string towerName, int index, float extra)
    {
        var name = index == 0 ? towerName + " Mesh" : towerName + " Mesh " + index;
        return Widen(original, ScriptableObject.CreateInstance<RenderPrefab>(), name, extra);
    }
        ...
        try
        {
            ...
            for (var index = 0; index < loaded.Length; index++)
            {
                ...
                if (blueSection)
                {
                    _report.Note(string.Format(
                        CultureInfo.InvariantCulture,
                        "{0}: applied the exact TrussArchBridge01 section map. Side arches, side "
                        + "decorations and other non-centre islands translate rigidly; each "
                ...
                if (index == 0)
                {
                    ...
                    DescribeProfile(name, source, scope);
                }
            ...
            var asset = AssetDatabase.user.AddAsset(
                AssetDataPath.Create("BridgePrefabGenerator", name, EscapeStrategy.None),
                geometry);
